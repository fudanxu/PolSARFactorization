...
nIter = 0;
while tryNo < nRepeat
    disp(['--->try no. ',num2str(tryNo)]);
    tryNo = tryNo+1;
    maxErr = 1;
    while(maxErr > differror)
        disp(['---->iter no. ',num2str(nIter)]);
        if issparse(X)
            % ===================== update V ========================
            if nzk < maxM
                Y = sum(U(idx,:).*V(jdx,:),2);
...
function [U,V] = NMF_init(X, k, origImg)
if (origImg)
    load U.mat;
    load V.mat;
    V=V0;
    V=V+1e-5*rand(size(V));
else
    [label, center] = litekmeans(X',k,'maxIter',10);
    %     [label, center] = kmeans(X',k,'replicate',3,'Options',statset('Display','final'));
    center = max(0,center);
    U = center';
    UTU = U'*U;
    UTU = max(UTU,UTU');
