...
if ischar(distance)
    ...
    if length(j) > 1
        ...
        error('litekmeans:AmbiguousDistance', 'Ambiguous ''Distance'' parameter value:  %s.', distance);
    
    elseif isempty(j)
        
        error('litekmeans:UnknownDistance', 'Unknown ''Distance'' parameter value:  %s.', distance);
    ...
    error('litekmeans:InvalidDistance', 'The ''Distance'' parameter value must be a string.');
...
if ischar(start)
    ...
    elseif isempty(k)
        ...
        error('litekmeans:MissingK', 'You must specify the number of clusters, K.');
...
elseif isnumeric(start)
    ...
    elseif (size(start,2) == 1 || size(start,1) == 1)
        ...
        error('litekmeans:MisshapedStart', 'The ''Start'' matrix must have the same number of columns as X.');
    ...
    elseif (k ~= size(center,1))
        ...
        error('litekmeans:MisshapedStart', 'The ''Start'' matrix must have K rows.');
    ...
    error('litekmeans:InvalidStart', 'The ''Start'' parameter value must be a string or a numeric matrix or array.');
...
if ~(isscalar(k) && isnumeric(k) && isreal(k) && k > 0 && (round(k)==k))
    ...
    error('litekmeans:InvalidK','X must be a positive integer value.');

elseif n < k
    
    error('litekmeans:TooManyClusters','X must have more rows than the number of clusters.');
